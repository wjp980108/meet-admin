 ...
 * @param axis 右击坐标信息
 */
function onShowContextmenu(sourceData: any, axis: Axis) {
...
function onMenuItemClick(item: ContextMenuItemClickEmitArg) {
  if (item.disabled)
    return;
...
<template>
  <transition name="el-zoom-in-center">
    ...
    >
      ...
      <span v-if="state.showArrow" class="el-popper__arrow" :style="{ left: `${state.arrowAxis}px` }" />
    </div>
  </transition>
</template>
